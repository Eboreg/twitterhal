...
class BaseDatabase:
    ...
    def add_key(self, name, type_, default=None, **default_kwargs):
        ...
        if name in self._schema:
            raise ValueError(f"Will not add duplicate key {name} to schema")
...
class RedisList(UserList):
    ...
    def push_to_cache(self):
        """Load and unpickle list contents from Redis to cache"""
        self.cache = self.list_type([pickle.loads(i) for i in self.redis.lrange(self.key, 0, -1)])

    def sync(self):
        """Pickle cache contents and save to cache"""
